...
PYK_regTest <- read.delim('2015.11.5-pyk_regtest.txt', fill = NA, header = F)
PYK_modeofInhibition <- read.delim('2015.11.5-pyk_inhibitionMode.txt', fill = NA, header = F)
PYK_noCoupling <- read.delim('2015.11.5-pyk_noCoupling.txt', fill = NA, header = F)

all_plate_names <- c("PYK_regTest", "PYK_modeofInhibition", "PYK_noCoupling")
...
INH_ID_mat <- matrix(c(rep("None", 12), rep(rep(c("Citrate", "Isocitrate"), each = 6), times = 6), rep("None", 12)),
                     ncol = 12, nrow = 8, dimnames = list(Row = LETTERS[1:8], Col = as.character(1:12)), byrow = T)
INH_ID <- melt(INH_ID_mat, value.name = "INH_ID")

INH_CONC <- melt(matrix(rep(c(0, 20, 15, 10, 6, 3, 1, 0) * 10/12, each = 12), ncol = 12, nrow = 8, dimnames = list(Row = LETTERS[1:8], Col = as.character(1:12)), byrow = T), value.name = "INH_CONC")
...
ADP_CONC <- melt(matrix(0.2 * 10/12, ncol = 12, nrow = 8, dimnames = list(Row = LETTERS[1:8], Col = as.character(1:12)), byrow = T), value.name = "ADP_CONC")
...
plate_design_track <- rbind(plate_design_track, INH_ID %>%
                              ...
                              left_join(ADP_CONC, by = c("Row", "Col")) %>%
                              left_join(ENZYME_CONC, by = c("Row", "Col")) %>%
                              mutate(plate = "PYK_regTest"))

# PYK_modeofInhibition

INH_ID_mat <- matrix(c(rep("Citrate", times = 12*6), rep("None", 12)),
                     ncol = 12, nrow = 7, dimnames = list(Row = LETTERS[1:7], Col = as.character(1:12)), byrow = T)
INH_ID <- melt(INH_ID_mat, value.name = "INH_ID")

INH_CONC <- melt(matrix(c(rep(c(1, 3, 6, 10, 15, 20) * 10/13, times = 12), rep(0, 12)), ncol = 12, nrow = 7, dimnames = list(Row = LETTERS[1:7], Col = as.character(1:12)), byrow = T), value.name = "INH_CONC")

PEP_CONC <- melt(matrix(c(rep(c(10, 6, 3, 1, 0.6, 0.3, 1), times = 6), rep(1, 7*6)) * 10/13, ncol = 12, nrow = 7, dimnames = list(Row = LETTERS[1:7], Col = as.character(1:12)), byrow = F), value.name = "PEP_CONC")
ADP_CONC <- melt(matrix(c(rep(0.2, 7*6), rep(c(1, 0.6, 0.3, 0.1, 0.06, 0.03, 0.2), times = 6)) * 10/13, ncol = 12, nrow = 7, dimnames = list(Row = LETTERS[1:7], Col = as.character(1:12)), byrow = F), value.name = "ADP_CONC")

ENZYME_CONC <- melt(matrix(c(rep(1, 12*6), rep(0, 12)), ncol = 12, nrow = 7, dimnames = list(Row = LETTERS[1:7], Col = as.character(1:12)), byrow = T), value.name = "ENZ_CONC")
...
plate_design_track <- rbind(plate_design_track, INH_ID %>%
                              ...
                              left_join(ADP_CONC, by = c("Row", "Col")) %>%
                              left_join(ENZYME_CONC, by = c("Row", "Col")) %>%
                              mutate(plate = "PYK_modeofInhibition"))

# PYK_noCoupling

INH_ID_mat <- matrix(c(rep("None", 6), rep("Citrate", 6*6), rep("None", 6)),
                     ncol = 6, nrow = 8, dimnames = list(Row = LETTERS[1:8], Col = as.character(1:6)), byrow = T)
INH_ID <- melt(INH_ID_mat, value.name = "INH_ID")

INH_CONC <- melt(matrix(rep(c(0, 20, 15, 10, 6, 3, 1, 0) * 10/13, each = 6), ncol = 6, nrow = 8, dimnames = list(Row = LETTERS[1:8], Col = as.character(1:6)), byrow = T), value.name = "INH_CONC")

PEP_CONC <- melt(matrix(rep(rep(c(0.2, 0.5), each = 3), 8) * 10/13, ncol = 6, nrow = 8, dimnames = list(Row = LETTERS[1:8], Col = as.character(1:6)), byrow = T), value.name = "PEP_CONC")
ADP_CONC <- melt(matrix(0.2 * 10/13, ncol = 6, nrow = 8, dimnames = list(Row = LETTERS[1:8], Col = as.character(1:6)), byrow = T), value.name = "ADP_CONC")

ENZYME_CONC <- melt(matrix(c(rep(1, 6*7), rep(0, 6)), ncol = 6, nrow = 8, dimnames = list(Row = LETTERS[1:8], Col = as.character(1:6)), byrow = T), value.name = "ENZ_CONC")
...
filter_setup <- data.frame(wellID = unique(scatter_data$wellID)) %>%
  mutate(is_decreasing = T,
         frac_filter_beginning = ifelse(grepl('noCoupling', wellID), 0.2, 0.2),
         frac_filter_end = ifelse(grepl('noCoupling', wellID), 0.6, 0.2))
...
for(one_plate in all_plate_names){
  ...
  print(
    ggplot(plate_data, aes(x = cumTime, y = A_change, color = factor(INH_CONC), group = wellID, alpha = -1*filtered)) + 
      geom_line(size = 1) + scatter_facet_theme + facet_grid(INH_ID ~ PEP_CONC) +
...
all_plate_summaries <- left_join(all_plate_rates, all_plates_PL %>% dplyr::select(-plate), by = "wellID") %>% tbl_df() %>%
  dplyr::select(wellID, plate, INH_ID:ENZ_CONC, rate = slope, PL)
...
all_plates_merge <- all_plate_summaries %>%
  ...
  group_by(plate, INH_ID, INH_CONC, PEP_CONC, ADP_CONC, ENZ_CONC) %>% mutate(Rep = 1:n()) %>%
  group_by(plate, PEP_CONC, ADP_CONC) %>% 
...
for(one_plate in all_plate_names){
  ...
  print(
    ggplot(plot_data, aes(x = factor(INH_CONC), y = relative_rate, group = Rep)) + 
      ...
      barplot_theme + facet_grid(INH_ID ~ PEP_CONC) +
  ...
  ggplot(plot_data, aes(x = factor(INH_CONC), y = frac_max)) + 
      geom_boxplot() + 
      barplot_theme + facet_grid(INH_ID ~ PEP_CONC) +
      scale_x_discrete("Inhibitior Concentration (mM)") + scale_y_continuous("Fraction of ``No Inhibitor``")
  ggsave(paste0(one_plate, "_frac_max_boxplot.pdf"), height = 8, width = 10)
