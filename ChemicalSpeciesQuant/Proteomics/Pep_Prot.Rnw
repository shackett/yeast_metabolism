...
for(a_pep_n in 1:n_p){
  ...
  if(all(names(table(table(peptide_model$Condition))) <= 1)){
    ...
    names(cond_est) <- peptide_model$Condition
      ...
      if(length(peptide_model[,1]) < length(unique(peptide_model$Condition)) + length(unique(peptide_model$totalBlocking))){
        ...
        cond_est <-  summary(gls_model)$coef + ifelse(names(summary(gls_model)$coef) == "(Intercept)", 0, gls_model$coef[1]); names(cond_est) <- unique(peptide_model$Condition)
          ...
          mixed_effects_model <- try(lme(RA ~ factor(Condition), weights = ~var_fit, data = peptide_model, random = ~ 1 | totalBlocking), silent = TRUE)
          
          grep('Error', mixed_effects_model[[1]][1]) == 1
          
          mixed_effects_model <- lme(RA ~ factor(Condition), weights = ~var_fit, data = peptide_model, random = ~ 1 | totalBlocking)
  ...
  model.matrix(RA ~ factor(Condition), data = peptide_model)
  ...
  if(sum(table(peptide_model$Condition) >= 2) >= 5){
    ...
    #### inflate each residual by nreps/(nreps - 1)
  ...
  precision_estimates[a_pep_n,] <- cond_summary$RA
