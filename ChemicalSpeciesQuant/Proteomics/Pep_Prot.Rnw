...
cor_plot <- cor_plot + scale_x_continuous(name = "Spearman Correlation", expand = c(0,0), limits = c(-1,1), breaks = seq(floor(min(pt_corrs$correlation)*10)/10, ceiling(max(pt_corrs$correlation)*10)/10, 0.2)) + scale_y_discrete(name = "Counts", expand = c(0,0), breaks = (0:ceiling(plotted_count_max/5))*5) + geom_histogram(colour = "aliceblue", fill = "limegreen", binwidth = 0.04)
cor_plot
ggsave(plot = cor_plot, "PTspearmans.pdf", height = 12, width = 16)
