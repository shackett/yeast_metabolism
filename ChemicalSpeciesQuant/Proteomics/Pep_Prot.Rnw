...
\delta^{o}_{j} &= log_{2}(\text{med}[P^{L}_{j}] - \text{med}[P^{H}_{j}])\notag\\
...
initial_offset = log2(apply(good_light, 2, median, na.rm = TRUE)/apply(good_heavy, 2, median, na.rm = TRUE))
conc_diff <- conditions$logSF[sapply(colnames(good_light), grep, x = conditions$condition[!(conditions$condition %in% c("p0.05H1", "p0.05H2"))], ignore.case = TRUE)]

abundMat <- abundMat + rep(1, times = length(abundMat[,1])) %*% t(conc_diff - initial_offset)
...
@
\caption{Concentration of proteins (mg) per $\mu$L cellular volume}
\label{prot_cellular_conc}
\end{figure}

\begin{figure}[h!]
<<prot_frac_DW, echo = FALSE, cache = TRUE>>=
dw_plot <- ggplot(dry_weight_DF, aes(x = factor(condition), y = ProteinDW_fraction, col = limitation)) + theme(axis.text.x = element_text(size = 4, face = "bold"))
...
rownames(impSVD_pcs) <- rownames(impute_abund_thresh)

heatmap.2(t(impSVD_pcs), Colv = FALSE, Rowv = FALSE, trace = "none", col = greenred(100), dendrogram = "none", colsep = c(5,10), denscol = "white")
@
...
factor_plot <- ggplot(pc_plot_df, aes(x = condition, y = value, group = cond, col = PC)) + facet_wrap(~ PC, ncol =1, scales = "free_y")
