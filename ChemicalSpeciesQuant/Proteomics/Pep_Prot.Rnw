...
hist(dispersion_table$dispersion_SW_test, breaks = 50)
...
dispersion_table[, median(log2(dispersion_adj), na.rm = TRUE), by = sample_fraction]
...
ggplot(dispersion_adj_df, aes(x = shrunk_estimate, y = log_dispersion_SD, color = nvals)) + geom_point(size = 3)
