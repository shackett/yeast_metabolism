...
lightIC <- lightIC[,good_experimental_samples]; heavyIC <- heavyIC[,good_experimental_samples]; PepMatrix <- PepMatrix[,good_experimental_samples]
...
for(inst in unique(refSamples$Instrument)){
  ...
  for(a_sample in 1:length(refSamples$SampleName[refSamples$Instrument == inst])){
    ...
    bioRepPlot <- rbind(bioRepPlot, data.frame(experimental = logLight_redvec, reference = logHeavy_redvec, averageIC = avgSignal_redvec, variance = (logLight_redvec - avgSignal_redvec)^2*2,Instrument = inst, Set = refSamples$Set[refSamples$Instrument == inst][a_sample]))
...
barplot_theme <- theme(text = element_text(size = 20, face = "bold"), title = element_text(size = 25, face = "bold"), panel.background = element_blank(), legend.position = "none", 
  panel.grid.minor = element_blank(), panel.grid.major.y = element_blank(), axis.ticks.x = element_blank(), axis.line = element_line()) 
...
peptide_collapse_bio <- model.matrix(~ factor(Condition), data = proteomicsBlocking)
peptide_collapse_full <- model.matrix(~ factor(Condition) + factor(Instrument)*factor(Set)*factor(Block), data = proteomicsBlocking)

peptide_collapse_blocking <- model.matrix(~ factor(Instrument)*factor(Set)*factor(Block) + 0, data = proteomicsBlocking)
colnames(peptide_collapse_blocking) <- NULL

###########

inst_set_level_joined <- apply(proteomicsBlocking, 1, function(x){
    paste(x[names(x) %in% c("Instrument", "Set")], collapse = "_")
    }); inst_set_levels <- unique(inst_set_level_joined)
inst_block_level_joined <- apply(proteomicsBlocking, 1, function(x){
    paste(x[names(x) %in% c("Instrument", "Set", "Block")], collapse = "_")
    }); inst_block_levels <- unique(inst_block_level_joined)


peptide_collapse_bio <- matrix(0, ncol = n_c, nrow = n_e)
peptide_collapse_set <- matrix(0, ncol = length(inst_set_levels), nrow = n_e)
peptide_collapse_block <- matrix(0, ncol = length(inst_block_levels), nrow = n_e)
for(exp in 1:n_e){
  peptide_collapse_bio[exp, conditions == proteomicsBlocking$Condition[exp]] <- 1
  peptide_collapse_set[exp, inst_set_levels == inst_set_level_joined[exp]] <- 1
  peptide_collapse_block[exp, inst_block_levels == inst_block_level_joined[exp]] <- 1
  }
colnames(peptide_collapse_bio) <- conditions; colnames(peptide_collapse_set) <- inst_set_levels; colnames(peptide_collapse_block) <- inst_block_levels

anova(lm(abundMat[x,] ~ peptide_collapse_full), lm(abundMat[x,] ~ data.frame(peptide_collapse_bio, peptide_collapse_set, peptide_collapse_block)))
anova(lm(abundMat[x,] ~ peptide_collapse_full), lm(abundMat[x,] ~ peptide_collapse_bio + peptide_collapse_set + peptide_collapse_block + 0))

z <- lm(abundMat[x,] ~ peptide_collapse_bio + peptide_collapse_set + peptide_collapse_block + 0)

peptide_model <- data.frame(RA = abundMat[x,], proteomicsBlocking, totalBlocking = inst_block_level_joined)
peptide_model <- peptide_model[!is.na(peptide_model$RA),]
peptide_model$Condition <- as.factor(peptide_model$Condition)
peptide_model$totalBlocking <- as.factor(peptide_model$totalBlocking)

lmer(RA ~ Condition + (Condition | totalBlocking), peptide_model)

lmer(RA ~ Condition + (Condition | totalBlocking) + (Condition | Set) + (Condition | Block), peptide_model)



anova(lm(abundMat[x,] ~ peptide_collapse_full), lm(abundMat[x,] ~ peptide_collapse_bio))
...
########
  ...
  proteomicsBlocking[,colnames(


model.matrix(~ factor(inst_set_level_joined) + 0, data = proteomicsBlocking)





anova(lm(abundMat[x,] ~ peptide_collapse_bio), lm(abundMat[x,] ~ peptide_collapse_bio2))
lm(peptide_collapse_bio2 ~ 





contr.treatment(10, 1, contrasts = FALSE)



fewMV <- c(1:length(abundMat[,1]))[rowSums(is.na(abundMat)) <= 25]

pblockSig <- sapply(fewMV, function(x){
  anova(lm(abundMat[x,] ~ peptide_collapse_bio), lm(abundMat[x,] ~ peptide_collapse_full))[2,6]
  })
predict(lm(abundMat[x,] ~ peptide_collapse_full), diag(25))


sig_block_effect <- fewMV[order(pblockSig)[1:50]]
for(i in 1:length(sig_block_effect)){
  print(plot(abundMat[sig_block_effect[i],], col = factor(proteomicsBlocking$Instrument)))
  }

#determine the significance of 


#calling single outlier points
cooks.distance(lm(abundMat[sig_block_effect[1],] ~ peptide_collapse_full))

#look at euclidian distance between all sets to see if one instrument by set is seriously funky - such as row 18

#within set sum of squares - flag if sd of SS is large
subset <- data.frame(RA = abundMat[x,], cond = proteomicsBlocking$Condition, inst = proteomicsBlocking$Instrument, set = proteomicsBlocking$Set)
subset_cast <- acast(subset, cond ~ inst + set, value.var = "RA")
dist(t(subset_cast), diag = TRUE)
subset_cast - apply(subset_cast, 2, mean, na.rm = TRUE)


# possible effects
# whole instruments * set
# whole instrument * block * set
# single observations - purely outliers

plot(abundMat[sig_block_effect[1],], col = factor(proteomicsBlocking$Instrument))
