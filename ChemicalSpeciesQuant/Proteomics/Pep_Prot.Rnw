...
for(a_pep_n in 1:n_p){
  ...
  peptide_model$RA - peptide_model$fitted
  
  if(sum(table(peptide_model$Condition) >= 2) < 5 & all(peptide_model$outlier_type == "peptide")){ #if peptide was flagged due to inconsistency between replicates and there are few examples where a peptide was ascertained in multiple conditions, then discard the peptide
    next
    }
