...
for(a_pep_n in 1:n_p){
  ...
  peptide_model <- peptide_model[!is.na(peptide_model$RA),]
  peptide_model$var_fit = sapply(1:length(peptide_model[,1]), function(x){predict(var_spline_inst[[peptide_model$Instrument[x]]], peptide_model$averageIC[x])$y})
  
  mixed_effects_model <- lme(RA ~ Condition, weights = ~var_fit, data = peptide_model, random = ~ 1 | totalBlocking)
