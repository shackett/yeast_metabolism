...
dispersion_ks.test2 <- dispersion_ks.test <- rep(NA, n_p)
...
for(a_pep_n in 1:n_p){
  ...
  ### Adjusting the variance between
  if(sum(table(peptide_model$Condition) >= 2) >= 5){
    ...
    #dispersion_adj[a_pep_n] 
    dispersion_adj[a_pep_n] <- 1/(length(repeated_conds[,1])/sum(((repeated_conds$RA - repeated_conds$fitted)^2*(repeated_conds$nreps/(repeated_conds$nreps - 1))*(1/repeated_conds$var_fit))))
    dispersion_ks.test2[a_pep_n] <- ks.test((repeated_conds$RA - repeated_conds$fitted)/sqrt(repeated_conds$var_fit*dispersion_adj[a_pep_n]), pnorm)$p
...
hist(dispersion_ks.test, breaks = 50)
