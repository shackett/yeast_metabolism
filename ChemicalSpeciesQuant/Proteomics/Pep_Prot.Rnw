...
opts_knit$set(progress = TRUE, verbose = TRUE, stop_on_error = 1)
...
### all quantifiable peptides must be above threshold for both light and heavy peptides
lightIC[is.na(heavyIC)] <- NA; heavyIC[is.na(lightIC)] <- NA
PepMatrix[is.na(heavyIC)] <- NA
...
LHscatter + geom_hex(bins = 200) + ggtitle("signal:noise ~ log2 average IC")
@
...
LHscatter + geom_hex(bins = 200) + ggtitle("log2 signal:noise ~ log2 average IC")
@


<<preOutlierSNspline>>=
ggplot() + facet_grid(~ instrument) + geom_line(data = var_spline_plot$spline, aes(x = x, y = y, color = "RED", size = 2)) + scatter_theme + scale_x_continuous("Average log10 SN") + scale_y_continuous("average pooled standard deviation") + scale_color_identity() + scale_size_identity() + geom_point(data = var_spline_plot$bins, aes(x = log10(SN), y = MLE_var, color = "blue", size = 3)) + ggtitle("average standard deviation by pooled signal noise of raw data")
@
...
sampleChemoInfo <- conditions[sapply(proteomicsBlocking$Condition, function(x){c(1:length(conditions[,1]))[conditions$condition == x]}),] #align chemostat conditions 
...
#### filter a peptide on an instrument if the light and heavy peptide are highly discordant - this can be seen as a gross mismatch between the p0.05 H and L sample on each instrument ###
#### because these samples will be missing on some instruments*peptides impute all missing values with knn.imputation and then flag which peptides are discordant on a given instrument run ###

library(impute)

impute_abundMat <- impute.knn(as.matrix(abundMat), k = 10, colmax = 0.9, rowmax = 1 - quality_frac)$data
tech_diff_threshold <- 5

tech_diff_boolean <- abs(impute_abundMat[,proteomicsBlocking$Condition == "p0.05"]) > tech_diff_threshold
colnames(tech_diff_boolean) <- inst_set_level_joined[proteomicsBlocking$Condition == "p0.05"]
...
for(a_pep_n in 1:n_p){
  ...
  ### call a set as outliers if p0.05L greatly departs from p0.05H ###
  
  peptide_model$is_outlier[peptide_model$setBlocking %in% colnames(tech_diff_boolean)[tech_diff_boolean[a_pep_n,]]] <- T
  peptide_model$outlier_type[peptide_model$setBlocking %in% colnames(tech_diff_boolean)[tech_diff_boolean[a_pep_n,]]] <- "technicalRepMismatch"
  
  ### if there are few samples left ditch the peptide ###
  
  if(sum(peptide_model$is_outlier == F) < n_e*quality_frac){
    filtered_peps[a_pep_n, !is.na(abundMat[a_pep_n,])] <- TRUE
    outlier_type[a_pep_n, !is.na(abundMat[a_pep_n,])] <- "technicalRepMismatch"
    next
    }
  
  
  ### Prune individual points if they are strong outliers based upon a bonferroni-corrected wald test
  wald_p <- 1 - abs(pnorm((peptide_model$RA[!peptide_model$is_outlier] - mean(peptide_model$RA[!peptide_model$is_outlier]))/sd(peptide_model$RA[!peptide_model$is_outlier])) - 0.5)*2
  peptide_model$outlier_type[!peptide_model$is_outlier][wald_p < 0.05/length(wald_p) & abs(peptide_model$RA[!peptide_model$is_outlier]) > 5] <- "point"
  peptide_model$is_outlier[!peptide_model$is_outlier][wald_p < 0.05/length(wald_p) & abs(peptide_model$RA[!peptide_model$is_outlier]) > 5] <- TRUE
  
  
  ### if a peptide has consistently high variance rather than doing aggresive pruning, the peptides elevated variance will be accounted for at the level of expected precision
  ...
  if(totally_inconsistent){
    ...
    peptide_model$is_outlier[peptide_model$outlier_type == "sparse"] <- T
  }
...
<<pooled_variance_setup, dependson = "dataImp", cache = TRUE>>=
...
<<LHfiltered, fig.height = 10, fig.width = 15>>=
...
<<pooled_variance_IC, dependson = "pooled_variance_setup">>=
##### Fitting Var(IC) #####
...
for(inst in unique(bioRepPlot$Instrument)){
  # calculate a variance/precision kernal over instrument*IC
  instSubset <- bioRepPlot[bioRepPlot$Instrument == inst & !(bioRepPlot$outlier_type %in% c("block", "point", "technicalRepMismatch")),]
  binsize <- floor(length(instSubset[,1])/nbins)
...
ggplot() + facet_grid(~ instrument) + geom_line(data = var_spline_plot$spline, aes(x = x, y = y, color = "RED", size = 2)) + scatter_theme + scale_x_continuous(" average log10-SN") + scale_y_continuous("average pooled standard deviation") + scale_color_identity() + scale_size_identity() + geom_point(data = var_spline_plot$bins, aes(x = log10(SN), y = MLE_var, color = "blue", size = 3)) + ggtitle("Average binned variance versus log signal:noise")
@

<<pooled_variance_SN_high>>=
ggplot() + facet_grid(~ instrument) + geom_line(data = var_spline_plot$spline, aes(x = x, y = y, color = "RED", size = 2)) + scatter_theme + scale_x_continuous(" average log10-SN") + scale_y_continuous("average pooled standard deviation", limits = c(0,1)) + scale_color_identity() + scale_size_identity() + geom_point(data = var_spline_plot$bins, aes(x = log10(SN), y = MLE_var, color = "blue", size = 3)) + ggtitle("Average binned variance versus log signal:noise, ceiling of sd = 1")
@
...
bioRepPlot_outlier_plot + geom_bar(binwidth = 5) + scale_y_continuous(expand = c(0,0)) + scale_x_continuous(expand = c(0,0)) + ggtitle("Signal:Noise of pruned and valid peptides")
@




<<point_disp_Estimation, dependson = c("pooled_variance, pooled_variance_SN"), cache = TRUE>>=
...
for(a_pep_n in 1:n_p){
  ...
  peptide_model <- peptide_model[!is.na(peptide_model$RA),]
  peptide_model <- peptide_model[!(peptide_model$outlier_type %in% c("block", "point", "technicalRepMismatch")),]
  ...
  if(a_pep_n %% 500 == 0){
    print(paste(round(a_pep_n/n_p * 100, 2), "% done"))
    }
  
}

#(1:nrow(point_estimates))[apply(point_estimates, 1, max, na.rm = T) == max(apply(point_estimates, 1, max, na.rm = T))]
#(1:ncol(point_estimates))[apply(point_estimates, 2, max, na.rm = T) == max(apply(point_estimates, 2, max, na.rm = T))]
...
dispersion_table$nvals = rowSums(!is.na(abundMat))
dispersion_table[, sample_fraction:= fraction_plot$label[fraction_plot$max >= nvals][1], by = nvals]

#ggplot(dispersion_table, aes(x = log2(dispersion_adj))) + geom_bar(binwidth = 0.3, fill = "RED")
...
<<peptide_pointAndPrecisionEst, dependson = "point_disp_Estimation">>=
######
...
<<EMprep, dependson = "peptide_pointAndPrecisionEst">>=	
...
#alpha_pres[alpha_pres == 1][colSums(mixing_fract[,alpha_pres == 1]) == 0] <- 0
...
impute_abund  <- impute.knn(as.matrix(t(prot_abund_final)), k = 10, rowmax = 0.8)$data

write.output(impute_abund, "proteinAbundance.tsv")
...
plot(whole_data_logL, xlab = "Iteration", ylab = "log change in log-likelihood", pch = 16)
@
...
valid_compartments <- colnames(gfp)[GFPlocalizationCols][rowSums(geneLocal, na.rm = T) > 15]
geneLocal <- geneLocal[rowSums(geneLocal, na.rm = T) > 15,]
rownames(geneLocal) <- valid_compartments
...
offset_projection <- trans_svd$u %*% diag(trans_svd$d[1:n_trans_pc]) %*% t(trans_svd$v[sapply(c("ClimD.25", "PlimD.05.2") , function(cond){c(1:length(transcript_brauer[1,]))[colnames(transcript_brauer) == cond]}),])
transc_renorm_offset <- offset_projection[,2] - offset_projection[,1]

transcript_brauer <- transcript_brauer - transc_renorm_offset

transcript_brauer <- transcript_brauer[,grep('^S', colnames(transcript_brauer), invert = T)]


transcript.condition <- read.delim("TranscriptomicsChemostatsActualDR.tsv") # import chemostat dilution rates
...
DR_change_mat <- matrix(0, nrow = nrow(transcript.condition), ncol = length(prot_cond[,1]))
...
clust_plot_df1$condition_name <- rownames(impute_abund)[clust_plot_df1$condition]
@

\begin{figure}[h!]
<<protclustplot, dependson = "prot_clust", echo = FALSE>>=

scatter_facet_theme <- theme(text = element_text(size = 23, face = "bold"), title = element_text(size = 25, face = "bold"), panel.background = element_rect(fill = "azure1"), legend.position = "right", panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(), panel.grid.major.y = element_line(size = 0.5, color = "black"), axis.text.x = element_text(angle = 90), strip.background = element_rect(fill = "coral")) 

clust_plotterer <- ggplot(clust_plot_df1, aes(x = condition_name, y = abundance, colour = dataType, size = clustersize/2)) + facet_wrap( ~ cluster, ncol = 4) + scale_size_area(guide = "none") + scale_colour_discrete(guide = "none")

clust_plotterer + geom_point() + scatter_facet_theme
@
...
clust_plot_df2$condition_name <- rownames(impute_abund)[clust_plot_df2$condition]
@
...
clust_plotterer <- ggplot(clust_plot_df2, aes(x = condition_name, y = abundance, colour = dataType, size = clustersize)) + facet_wrap( ~ cluster, ncol = 4) + scale_size_area(guide = "none") + scale_colour_discrete(guide = "none")
clust_plotterer + geom_point() + scatter_facet_theme
@
...
scatter_facet_theme <- theme(text = element_text(size = 23, face = "bold"), title = element_text(size = 25, face = "bold"), panel.background = element_blank(), legend.position = "right", panel.grid.minor = element_blank(), panel.grid.major = element_line(size = 0.25, color = "black", linetype = "longdash"), axis.text.x = element_text(angle = 90),
...
#print(divScatterPlot + geom_point(aes(size = 2)) + scale_x_continuous(name = expression(log[2] ~ "Transcript relative abundance")) + scale_y_continuous(name = expression(log[2] ~ "Protein relative abundance")) + scale_colour_discrete(guide = "none") + scale_fill_discrete(name = "Limitation") + scale_size_continuous(guide = "none") + geom_smooth(method = lm, aes(fill = factor(limitation), alpha = as.numeric(0))) + scale_alpha_continuous(guide = "none", range = c(0.2,0.2)))

print(divScatterPlot + geom_point(aes(size = 2)) + scale_x_continuous(name = expression(log[2] ~ "Transcript relative abundance")) + scale_y_continuous(name = expression(log[2] ~ "Protein relative abundance")) + scale_colour_discrete(guide = "none") + scale_fill_discrete(name = "Limitation") + scale_size_continuous(guide = "none") + geom_smooth(method = lm, aes(fill = NULL, alpha = as.numeric(0))) + scale_alpha_continuous(guide = "none", range = c(0,0)))

ggsave("PTgoodBadCompare.pdf", height = 15, width = 25)
@
