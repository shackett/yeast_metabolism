...
for(a_pep_n in 1:n_p){
  ...
  if(totally_inconsistent){peptide_model$is_outlier <- TRUE; totally_inconsistent <- FALSE}
  }
...
for(inst in unique(refSamples$Instrument)){
  ...
  if(length(logLight[1,]) == 1){
    logLight[filtered_peps[proteomicsBlocking$SampleName %in% refSamples$SampleName[refSamples$Instrument == inst]] == 1,1] <- NA
    logHeavy[filtered_peps[proteomicsBlocking$SampleName %in% refSamples$SampleName[refSamples$Instrument == inst]] == 1,1] <- NA
    }else{
      logLight[filtered_peps[,proteomicsBlocking$SampleName %in% refSamples$SampleName[refSamples$Instrument == inst]] == 1] <- NA
      logHeavy[filtered_peps[,proteomicsBlocking$SampleName %in% refSamples$SampleName[refSamples$Instrument == inst]] == 1] <- NA
      }
