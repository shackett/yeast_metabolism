...
for(a_pep_n in 1:n_p){
  ...
  bad_block_samples <- bad_block_samples[!is.na(bad_block_samples)]
  
  peptide_model$is_outlier[bad_block_samples] <- TRUE
  peptide_model$outlier_type[bad_block_samples] <- "block"
  
  if(totally_inconsistent){
    totally_inconsistent <- FALSE; peptide_model$outlier_type[is.na(peptide_model$outlier_type)] <- "peptide"}
  }
...
for(inst in unique(refSamples$Instrument)){
  ...
  for(a_sample in 1:length(refSamples$SampleName[refSamples$Instrument == inst])){
    ...
    logHeavy_redvec = logHeavy[,a_sample][is.finite(avgSignalSTD[,a_sample])]
    avgSignal_redvec = avgSignalSTD[is.finite(avgSignalSTD[,a_sample]),a_sample]
    logLight_redvec <- logLight_redvec + optimize(normFactor, c(-1, 1), logLight = logLight_redvec[is.na(outlier_type_redvec)], logHeavy = logHeavy_redvec[is.na(outlier_type_redvec)])$minimum
    
    bioRepPlot <- rbind(bioRepPlot, data.frame(experimental = logLight_redvec, reference = logHeavy_redvec, averageIC = avgSignal_redvec, variance = (logLight_redvec - avgSignal_redvec)^2*2,Instrument = inst, Set = refSamples$Set[refSamples$Instrument == inst][a_sample], outlier_type = outlier_type_redvec))
...
for(inst in unique(bioRepPlot$Instrument)){
  # calculate a variance/precision kernal over instrument*IC
  instSubset <- bioRepPlot[bioRepPlot$Instrument == inst & is.na(bioRepPlot$outlier_type),]
  binsize <- floor(length(instSubset[,1])/nbins)
  error_bins <- data.frame(val_mean = instSubset$averageIC[order(instSubset$averageIC)], light_val = instSubset$experimental[order(instSubset$averageIC)], bin = c(rep(1:(length(instSubset[,1]) %% nbins), each = binsize + 1), rep(((length(instSubset[,1]) %% nbins) + 1):nbins, each = binsize)))
...
for(a_pep_n in 1:n_p){
  ...
  peptide_model <- data.frame(RA = abundMat[a_pep_n,], proteomicsBlocking, totalBlocking = inst_block_level_joined, setBlocking = inst_set_level_joined, lightIC = log2(good_light[a_pep_n,]), heavyIC = log2(good_heavy[a_pep_n,]), averageIC = (log2(good_light[a_pep_n,]) + log2(good_heavy[a_pep_n,]))/2, outlier_type = outlier_type[a_pep_n,])
  peptide_model <- peptide_model[!is.na(peptide_model$RA),]
  peptide_model$sd_fit = sapply(1:length(peptide_model[,1]), function(x){predict(var_spline_inst[[peptide_model$Instrument[x]]], peptide_model$averageIC[x])$y})
    
  A = rnorm(1000, 0, 1)
  B = A*500 + rnorm(1000, 0, 200)
  W = c(rep(1, 500), rep(0.25, 500))
  test_frame = data.frame(A = A, B = B, W = W, E = 1)
  lme(B ~ A, weights = ~W, data = test_frame, random =  ~ 1 | E) # weights are by variance rather precision
  lm(B ~ A, weights = 1/W, data = test_frame)
  
  ### Prune whole sets if they are highly inconsistent with other sets (and more extreme / high variance) ### Not currently implemented due to the effectiveness of block-level filtering
  
  ### Fit a mixed effects model treating each condition as a fixed effect and all blocking factors as random effects to estimate fixed effects ####
  mixed_effects_model <- lme(RA ~ Condition, data = peptide_model[!peptide_model$is_outlier,], random = ~ 1 | totalBlocking)
  
  mixed_effects_model$resid
  
  plot(peptide_model$RA, col = factor(peptide_model$Instrument)
