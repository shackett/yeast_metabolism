...
\begin{figure}[h!]
<<protPlot1, dependson = "EMprep", cache = TRUE>>=
plot_protein_lattice(c(1:5), possibleMap, prot_abund_final, prot_prec, uniquePepMean, uniquePepPrecision, mixing_fract, conditions, 1)
@
\caption{relative protein abundance and peptide trends contributing to protein trend}
\end{figure}

\begin{figure}[h!]
<<protPlot2, dependson = "EMprep", cache = TRUE>>=
plot_protein_lattice(c(11:26), possibleMap, prot_abund_final, prot_prec, uniquePepMean, uniquePepPrecision, mixing_fract, conditions, 4)
@
\caption{relative protein abundance and peptide trends contributing to protein trend}
\end{figure}

\begin{figure}[h!]
<<protPlot3, dependson = "EMprep">>=
plot_protein_lattice(c(27,32,42,114,183,206,220,222,229,241,264,308), possibleMap, prot_abund_final, prot_prec, uniquePepMean, uniquePepPrecision, mixing_fract, conditions, 4)
@
...
<<ModifyTransc, dependson = "MVimputeData", cache = TRUE>>=
library(impute)
...
<<fileDump1, dependson = "c(mvmat_red, mv_impute, PTcor)", echo = FALSE, cache = TRUE>>=
#file dump
