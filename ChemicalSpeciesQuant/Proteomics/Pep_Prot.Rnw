...
<<normalizeProtAbund, dependson = "EMprep">>=
...
levels(pc_plot_df$PC) <- paste("PC", c(1:npc))
...
<<>>=
plot_protein_lattice(c(11:26), possibleMap, prot_abund_final, prot_prec, uniquePepMean, uniquePepPrecision, mixing_fract, 4)
@

<<>>=
plot_protein_lattice(c(27:42), possibleMap, prot_abund_final, prot_prec, uniquePepMean, uniquePepPrecision, mixing_fract, 4)
@


<<metGenerep, dependson = "EMprep", results = "asis">>=
...
gplot.hexbin(hexbin(x = shared_prot_pool, y = shared_trans_pool, xbins = 25), colramp = blue2red, xlab = "Protein Abundance", ylab = "Transcript Abundance")
@
...
<<fileDump1, dependson = "c(mvmat_red, mv_impute, PTcor)", echo = FALSE>>=
#file dump
...
if(unq_matches_only == TRUE){
write.table(prot_missingVal_desc, file = "run_Unq/mvMatrix.tsv", sep = "\t", row.names = TRUE, col.names = TRUE)
write.table(uq_binPepMatrix[apply(uq_binPepMatrix != 0, 1, sum) != 0,], file = "run_Unq/mvMatrixpeps.tsv", sep = "\t", row.names = TRUE, col.names = TRUE)

write.table(t(imput_abund_desc), file = "run_Unq/relAbundMatrix_desc.tsv", sep = "\t", row.names = TRUE, col.names = TRUE)
write.table(t(impute_abund), file = "run_Unq/relAbundMatrix.tsv", sep = "\t", row.names = TRUE, col.names = TRUE)
...
write.table(prot_missingVal_desc, file = "run_Deg/mvMatrix.tsv", sep = "\t", row.names = TRUE, col.names = TRUE)
write.table(uq_binPepMatrix[apply(uq_binPepMatrix != 0, 1, sum) != 0,], file = "run_Deg/mvMatrixpeps.tsv", sep = "\t", row.names = TRUE, col.names = TRUE)
write.table(t(imput_abund_desc), file = "run_Deg/relAbundMatrix_desc.tsv", sep = "\t", row.names = TRUE, col.names = TRUE)
write.table(t(impute_abund), file = "run_Deg/relAbundMatrix.tsv", sep = "\t", row.names = TRUE, col.names = TRUE)
