...
for(a_pep_n in 1:n_p){
  ...
  peptide_model <- data.frame(RA = abundMat[a_pep_n,], proteomicsBlocking, totalBlocking = inst_block_level_joined)
  ...
  peptide_model$is_outlier <- FALSE
  peptide_model$outlier_type <- NA
  ...
  peptide_model$is_outlier[wald_p < 0.05/length(wald_p)] <- TRUE
  peptide_model$outlier_type[wald_p < 0.05/length(wald_p)] <- "point"
  
  ### Prune blocks if they are highly inconsistent with the average of other replicates (more extreme and high variance) ###
  
  RA_blocks <- acast(peptide_model, Condition ~  Instrument + Set + Block, value.var = "RA")
  mean_euclid_dist <- data.frame(average_euclid_d = rep(NA, length(RA_blocks[1,])), mean_val = NA)
  for(j in 1:length(euclid_dist)){
    primary_set <- RA_blocks[!is.na(RA_blocks[,j]),j]
    compare_set <- apply(RA_blocks[!is.na(RA_blocks[,j]),-j], 1, mean, na.rm = TRUE)
    mean_euclid_dist[j,] <- c(mean((primary_set[!is.nan(compare_set)] - compare_set[!is.nan(compare_set)])^2), mean(primary_set))
    }
    
  block_prune <- TRUE
  RA_blocks <- acast(peptide_model, Condition ~  Instrument + Set + Block, value.var = "RA")
  mean_euclid_dist <- data.frame(average_euclid_d = rep(NA, length(RA_blocks[1,])), mean_val = NA, pruned = FALSE, index = 1:length(RA_blocks[1,]))

  while(block_prune == TRUE){
    
    for(j in c(1:length(mean_euclid_dist[,1]))[!mean_euclid_dist$pruned]){
      primary_set <- RA_blocks[!is.na(RA_blocks[,j]),j]
      compare_set <- apply(RA_blocks[!is.na(RA_blocks[,j]),-j], 1, mean, na.rm = TRUE)
      mean_euclid_dist[j,1:2] <- c(mean((primary_set[!is.nan(compare_set)] - compare_set[!is.nan(compare_set)])^2), mean(primary_set))
      }
    if(max(mean_euclid_dist$average_euclid_d) > 4){
      ...
      index_reorder <- index_reorder[index_reorder$average_euclid_d > 4 & index_reorder$average_euclid_d > max(index_reorder$average_euclid_d) - 2,]
     # mean_euclid_dist$pruned[index_reorder$index[which.max(abs(index_reorder$mean_val))]] <- TRUE
  ...
  ### Prune whole sets if they are highly inconsistent with other sets (and more extreme / high variance) ###
  plot(peptide_model$RA)
...
LS_fit <- lm(RA ~ Condition + totalBlocking, data = peptide_model)

cooks.distance(LS_fit) > 4/length(peptide_model[,1])




library(nlme)
y <- lme(RA ~ Condition, data = peptide_model, random = ~ 1 | totalBlocking)

library(lme4)
z <- lmer(RA ~ Condition + (1 | totalBlocking), peptide_model, REML = TRUE)
